...
def stop_motor():
    motor_controller.working = False
    motor_controller.start_motor()
    return { 'success': True }
    
# Server view to move the motor to next color zone
@app.route('/move_to_next')
def move_to_next():
    motor_controller.next_color_zone()
    return { 'success': True }
...
def motor_status():
    if(motor_controller.is_working()==True):
        ...
        return { 'status': "Motor is stopped" }
